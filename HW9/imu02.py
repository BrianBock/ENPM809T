	...
	while True:
		if (ser.in_waiting >0):
			...
			if count > 10:
				...
				error = counterFL-counterBR
				counterBR += error
				if  counterFL >= ticks or (diff>=rot-2 and diff<=rot+2) :
					pwm1.stop()
					
				if  counterBR >= ticks or (diff>=rot-2 and diff<=rot+2):
					pwm2.stop()


				if (counterBR >=ticks and counterFL >= ticks) or (diff>=rot-2 and diff<=rot+2):
					#gameover()
					avg = float(counterBR+counterFL)/2
					a = int(avg/0.25)
